...
- **Exceção 13 — Communication → Reservations (resolução de reserva por telefone do hóspede), ADR-029 (Fase 11, Checkpoint 1):** exceção estrita e específica, não uma extensão da Exceção 5 (mesmo par de contextos, propósito distinto — mirroring a relação entre ADR-026 e ADR-028). Communication pode consultar Reservations **exclusivamente** para resolver Reservation(s) `Confirmed` elegíveis a partir do telefone normalizado do remetente de uma mensagem inbound do WhatsApp, através do contrato público `IReservationByGuestPhoneReader` (`Reservations.Contracts`, implementado somente em `Reservations.Infrastructure`), com entrada `TenantId`/`GuestPhoneNormalized` e retornando uma lista de `ReservationCandidate(ReservationId, PropertyId, CheckInAt, CheckOutAt)` — nunca o agregado `Reservation`, nunca `GuestName`/`GuestPhone` novamente. Communication nunca referencia `Reservations.Domain`/`Application`/`Infrastructure`/`ReservationsDbContext` diretamente. Regra de elegibilidade: apenas `Confirmed` — `Cancelled`/`Closed` nunca são elegíveis, sem janela temporal (decisão explícita, não uma heurística inventada). 0 ou N candidatos nunca resultam em criação automática de Conversation — apenas exatamente 1 candidato resolve automaticamente; a desambiguação entre N candidatos é deferida a um checkpoint futuro. Ver ADR-029 para justificativa completa e lista integral de restrições.
- **Exceção 14 — AI Agent → Communication (histórico sanitizado de conversa), ADR-030 (Fase 11, Checkpoint 2):** exceção estrita e específica, não uma extensão informal da Exceção 3 (que cobre Tools executando capabilities via Application Service, nunca uma leitura dedicada). AI Agent pode consultar Communication **exclusivamente** para obter o histórico sanitizado de mensagens de uma Conversation, através do contrato público `IConversationHistoryReader` (`Communication.Contracts`, implementado somente em `Communication.Infrastructure`), com entrada `TenantId`/`ConversationId` e retornando uma lista de `ConversationHistoryMessage(MessageId, Direction, Content, OccurredAtUtc)` — nunca o agregado `Message`, nunca referência a `Reservation`, nunca `ProviderMessageId`/status/destino/motivo de falha, nunca `GuestPhone`, nunca `AccessCredential`, nunca payload de QR PIX. AI Agent nunca referencia `Communication.Domain`/`Application`/`Infrastructure`/`CommunicationDbContext` diretamente. Ordem cronológica determinística (`CreatedAtUtc` + `MessageId` como tie-breaker). Conteúdo já marcado `"[SENSITIVE CONTENT REDACTED]"` na escrita (ADR-028) é retornado como está; conteúdo de entrega PIX (ADR-025/027, que legitimamente persiste o QR real em `RenderedContent`) é redigido pelo próprio reader antes de retornar — sem alterar a persistência já homologada da Fase 10. Ver ADR-030 para justificativa completa e lista integral de restrições.
