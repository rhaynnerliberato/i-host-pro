...
### 5.5 Circuit Breaker HTTP — Decision Gate resolvido (ADR-031)

Investigação inicial (evidência direta de código) confirmou que a decisão "sem Polly" (Fase 9 CP2.2/2.3.3, reaplicada por precedência na Fase 11 CP7) era protegida por uma `ArchitectureTest` real e específica — `ExternalIntegrationsDependencyTests.Infrastructure_References_No_Resilience_Or_Polly_Package()`, uma checagem categórica de pacote, mais ampla que a justificativa original ("zero automatic retry framework"). Trazido de volta como Decision Gate isolado, **aprovado pelo usuário**: reabrir a decisão estritamente para HTTP circuit breaking, via `Microsoft.Extensions.Http.Resilience` (oficial .NET/Microsoft) — nunca um circuit breaker hand-rolled (explicitamente rejeitado), nunca retry/hedging/fallback/timeout adicional.

**Implementado** (ver ADR-031 para o registro completo da decisão):

- `AddCircuitBreaker` — e somente isso — adicionado ao pipeline HTTP de `AnthropicModelProvider` e `MetaWhatsAppMessagingProvider`, via `Microsoft.Extensions.Http.Resilience` (v10.9.0). `AutomaticProviderRetryFromResiliencePipeline=false`: nenhum retry é adicionado pelo pipeline — o retry único já homologado da Anthropic (`ModelTechnicalRetryCount=1`) permanece exatamente onde estava, inalterado; Meta continua com zero retry (`AutomaticMetaRetry=false`).
- Classificação de falha reaproveita, sem duplicar, a lógica já existente em cada provider: `AnthropicModelProvider.IsPermanentFailure`/`MetaFailureCodes` — 400/401/403/404 nunca abrem o circuito; timeout, erro de rede, 429 e 5xx contam. 429 foi decidido explicitamente como contável (representa capacidade do provider esgotada, nunca gera uma tentativa adicional).
- Um `BrokenCircuitException` (circuito aberto, rejeição local, zero I/O de rede) é capturado no mesmo bloco try/catch já existente de cada provider: para Anthropic, mapeado para `ModelProviderException` transitório com outcome `"CircuitOpen"` no contador `ai_agent.model_calls` já existente (nenhuma métrica nova); para Meta, mapeado para `ProviderFailureCategory.TransientProviderFailure` (mesma categoria de um 5xx real — nunca `DeliveryOutcomeUnknown`, já que nenhuma tentativa HTTP real ocorreu).
- Configuração externa (`AIAgent:Anthropic:CircuitBreaker`/`ExternalIntegrations:WhatsApp:Meta:CircuitBreaker`), duas classes de Options deliberadamente não compartilhadas entre os dois Bounded Contexts (mesma disciplina de isolamento de toda outra dependência cross-cutting). `ProductionCircuitBreakerThresholdsRequired=true` — nenhum default aqui é um SLA de Produção.
- Telemetria compartilhada (`CircuitBreakerTelemetry`, `BuildingBlocks.Infrastructure.Resilience`) — métrica pura, sem tipo Polly em sua própria assinatura, então não torna Resilience uma dependência transitiva de todo consumidor de `BuildingBlocks.Infrastructure`. Estados (`Opened`/`Closed`/`HalfOpened`) apenas, tags `provider`/`state` de baixíssima cardinalidade.
- `ArchitectureTest` **reescrita, nunca removida** (mandato explícito: "não transformar em teste inútil") — `Only_The_Two_Approved_Infrastructure_Projects_May_Reference_Resilience_Or_Polly`, agora solução inteira (mais ampla que a regra original, que só cobria um projeto), com exatamente dois nomes aprovados na allowlist.
- Health nunca depende do estado do circuit breaker — mesma separação já estabelecida para `Redis=Degraded` (CP2): um provider externo com circuito aberto é feature degradada, nunca incapacidade do processo.

**Testes** (determinísticos, zero rede real, zero API key): `AnthropicCircuitBreakerTests`/`MetaCircuitBreakerTests` (11 testes no total) provam, contra um `IHttpClientFactory` real construído a partir de um `ServiceCollection` real (nunca o `FakeHttpClientFactory` que os testes de contrato já existentes usam, que contorna o pipeline de resiliência inteiramente) — CLOSED permite chamadas; falhas consecutivas transitórias abrem o circuito e rejeitam localmente sem nova chamada HTTP; após `BreakDuration`, uma probe HALF-OPEN bem-sucedida fecha o circuito (Meta) e uma probe falha o reabre (Anthropic); um 4xx permanente nunca abre o circuito; uma única falha produz exatamente uma tentativa HTTP, nunca duas (`AutomaticHttpResilienceRetry=false`, comprovado). Mais 2 testes de mapeamento (`AnthropicModelProviderTests`/`MetaWhatsAppMessagingProviderTests`) provando que `BrokenCircuitException` é convertido corretamente pelo próprio provider. `RetryAmplification=false` é uma consequência lógica direta do resultado por-chamada já comprovado (2 chamadas de `GenerateAsync` pelo retry de aplicação, cada uma delas já provada gerar zero I/O real quando o circuito está aberto) — não exige um teste combinado dedicado.

`HandRolledHttpCircuitBreaker=false`. `HttpCircuitBreakerDecision=APPROVED_OFFICIAL_RESILIENCE_PIPELINE`. `MicrosoftExtensionsHttpResilienceAllowed=true`. `OfficialHttpCircuitBreakerImplemented=true`. `AnthropicCircuitBreakerImplemented=true`. `MetaCircuitBreakerImplemented=true`.
...
- ~~Circuit breaker HTTP adicional para Anthropic/WhatsApp~~ — implementado (§5.5, ADR-031) após aprovação específica do Decision Gate.
