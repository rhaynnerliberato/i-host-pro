...
**Status:** Em andamento. Checkpoint 0 (Architecture & Product Decision Gate, read-only), Checkpoint 1 (Guest Operations Foundation), Checkpoint 2 (Check-in/Checkout Core) e Checkpoint 3 (Early Check-in / Late Checkout Core) concluídos — novo Bounded Context Guest Operations, `ReservationStatus.Closed`, comando cross-context `CloseReservation`, eventos `GuestCheckedIn`/`GuestCheckedOut`, segundo orquestrador de Workflow, primeiro endpoint HTTP público (`GuestOperations.Api`, check-in/checkout), gatilho de criação via coreografia (`ReservationCreated`), estratégia de upgrade para Reservations preexistentes; e, no Checkpoint 3, os agregados `EarlyCheckInRequest`/`LateCheckoutRequest` com decisão automática, as duas primeiras exceções síncronas cross-context de Guest Operations (`IReservationScheduleReader`/`ICleaningReadinessReader`, Architecture Principles §14 Exceções 7/8), reagendamento real da Reservation via dois novos orquestradores de Workflow, quatro novos Integration Events, e a reação real (auditoria, sem mutação de agenda inventada) de Housekeeping — tudo provado por testes reais (unitários, arquitetura, HTTP real com JWT real, e E2E via RabbitMQ/Worker/Postgres reais). `Percentage` (cálculo de desconto/acréscimo proporcional) permanece oficialmente não suportado, pendente de um domínio de pricing futuro. Credencial de acesso permanece deferida (sub-gate de segurança pendente antes da homologação final da Fase 10). Portaria/Front Desk (Checkpoint 4) e PIX/Payment (Checkpoint 5, `ADR-025` reservada) ainda não implementados — deferidos a checkpoints futuros.
