...
**Status:** Em andamento. Checkpoint 0 (Architecture & Product Decision Gate, read-only), Checkpoint 1 (Guest Operations Foundation), Checkpoint 2 (Check-in/Checkout Core), Checkpoint 3 (Early Check-in / Late Checkout Core) e Checkpoint 4 (Portaria Notification Foundation) concluídos — novo Bounded Context Guest Operations, `ReservationStatus.Closed`, comando cross-context `CloseReservation`, eventos `GuestCheckedIn`/`GuestCheckedOut`, segundo orquestrador de Workflow, primeiro endpoint HTTP público (`GuestOperations.Api`, check-in/checkout), gatilho de criação via coreografia (`ReservationCreated`), estratégia de upgrade para Reservations preexistentes; no Checkpoint 3, os agregados `EarlyCheckInRequest`/`LateCheckoutRequest` com decisão automática, as duas primeiras exceções síncronas cross-context de Guest Operations (`IReservationScheduleReader`/`ICleaningReadinessReader`, Architecture Principles §14 Exceções 7/8), reagendamento real da Reservation via dois novos orquestradores de Workflow, quatro novos Integration Events, e a reação real (auditoria, sem mutação de agenda inventada) de Housekeeping; e, no Checkpoint 4, um Architecture & Product Decision Gate read-only prévio, o novo agregado `FrontDeskContact` (Property Management, um contato ativo por Condomínio), a nona exceção síncrona cross-context (`IFrontDeskContactReader`, Communication → Property Management, ADR-026 dedicada, Architecture Principles §14 Exceção 9), a extensão factual de ADR-019 (`GuestName`), três novos consumidores de Communication para notificação operacional de Portaria (sem provider real, sem conteúdo de mensagem real, sem role/portal/acknowledgement de Portaria) — tudo provado por testes reais (unitários, arquitetura, HTTP real com JWT real, e E2E via RabbitMQ/Worker/Postgres reais). `Percentage` (cálculo de desconto/acréscimo proporcional) permanece oficialmente não suportado, pendente de um domínio de pricing futuro. Credencial de acesso permanece deferida (sub-gate de segurança pendente antes da homologação final da Fase 10) — confirmado sem qualquer relação com a notificação de Portaria. PIX/Payment (Checkpoint 5, `ADR-025` reservada) ainda não implementado — deferido a checkpoint futuro.
