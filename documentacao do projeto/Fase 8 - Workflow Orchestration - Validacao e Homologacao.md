...
#### 5.13.9 Correção transversal pós-homologação (ADR-020)

**Registrado posteriormente, sem reabrir esta fase funcionalmente.** Durante a Fase 9 (Comunicação e Integrações do MVP), um teste real de transporte já homologado desta fase (`CreateCleaningForReservationWorkflowRoundTripTests`) passou a falhar de forma reproduzível ao se adicionar um quarto Bounded Context consumindo `ReservationCreated` no mesmo processo `IHostPro.Worker`. A investigação revelou um defeito latente, transversal e anterior a esta fase: por padrão, o Wolverine combina em uma única cadeia de execução todos os handlers descobertos para o mesmo tipo de mensagem, independentemente do número de filas/listeners distintos configurados — o próprio consumidor `ReservationCreated` desta Fase 8 (Workflow) já compartilhava esse risco estrutural com Housekeeping e Dashboard (Fases 6 e 7), mesmo antes de Communication existir. O defeito foi corrigido de forma transversal, com `AddStickyHandler` aplicado a cada fila afetada em `IHostPro.Worker/Program.cs`, e registrado em ADR-020. Ver ADR-020 (Isolamento de Handler Chains do Wolverine para Fan-out entre Bounded Contexts) para a investigação completa, a prova estrutural e de transporte real (reexecutando o mesmo teste desta fase), e o escopo exato da correção — que não alterou nenhum comportamento funcional desta fase, apenas a composição interna de handler chains do Wolverine.
