...
/// <summary>
/// Isolates AI Agent's own tenant execution, persistence and transaction
/// ownership from Wolverine's message-transport dependency graph — the
/// local, per-Bounded-Context boundary ADR-016 requires for ANY persistent
/// Wolverine consumer that touches a <c>BaseDbContext</c>-derived,
/// tenant-filtered <c>DbContext</c> (mirrors
/// <c>ICommunicationMessageExecutionScope</c>/<c>IDashboardMessageExecutionScope</c>/
/// <c>IHousekeepingMessageExecutionScope</c>/<c>IReservationsMessageExecutionScope</c>
/// exactly — ADR-016's own "Alternativa Rejeitada 2" already establishes
/// this is deliberately duplicated per context, never a shared generic
/// abstraction).
///
/// The single implementation of this interface is the ONLY class in AI
/// Agent authorized to hold an <c>IServiceScopeFactory</c> — see the
/// architecture test enforcing this boundary. The Wolverine transport
/// adapter depends on this interface ONLY — never on
/// <c>AIAgentDbContext</c> or the message processor directly.
/// </summary>
public interface IAIAgentMessageExecutionScope
{
