...
/// <inheritdoc cref="StartOwnCleaningInspectionCommand"/>
public sealed class StartOwnCleaningInspectionCommandHandler : ICommandHandler<StartOwnCleaningInspectionCommand, CleaningResult>
{
    ...
    public StartOwnCleaningInspectionCommandHandler(
    ...
    public async ValueTask<Result<CleaningResult>> Handle(StartOwnCleaningInspectionCommand command, CancellationToken cancellationToken) =>
        await _executor.ExecuteAsync(async () =>
        {
            ...
            _auditWriter.Record(CleaningAuditEntry.Create(
                ...
                "cleaning_inspection_started", ["status"], now));

            _eventCollector.Enqueue(new CleaningInspectionStarted
            {
