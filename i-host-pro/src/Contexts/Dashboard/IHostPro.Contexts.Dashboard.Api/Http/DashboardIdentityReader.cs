...
/// <summary>
/// Canonical, single-source claim extraction for <see cref="Controllers.DashboardController"/>
/// — no genuine, reusable abstraction for this exists outside <c>Identity.Api</c>,
/// which this project must never reference (mirrors
/// <c>Reservations.Api.Http.ReservationsIdentityReader</c>'s own independent
/// copy, limited to the two claims this context actually needs).
///
...
/// its own use.
/// </summary>
public static class DashboardIdentityReader
{
    private const string SubClaimType = "sub";
    private const string TenantIdClaimType = "tenant_id";

    public static bool TryRead(ClaimsPrincipal user, out AuthenticatedDashboardIdentity identity)
    {
        identity = default;
        ...
        identity = new AuthenticatedDashboardIdentity(userId, tenantId);
        return true;
    }
