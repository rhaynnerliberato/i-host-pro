...
/// <summary>
/// Front Desk ("Portaria") operational notification for a real guest
/// check-in (Fase 10, Checkpoint 4 — Portaria Notification Foundation).
/// Mirrors <see cref="ReservationCreatedCommunicationProcessor"/>'s
/// structure (idempotency, Template resolution, Message lifecycle,
/// connector dispatch) with a different recipient: the Condominium's
/// configured front desk contact (ADR-026, synchronous exception #9),
/// resolved from <see cref="GuestCheckedIn.PropertyId"/> — never the guest.
///
/// If no active front desk contact is configured for the Property's
/// Condominium (or the Property has no Condominium at all), this is a
/// deliberate no-op: no <see cref="Message"/> is ever created, logged with
/// reason code <c>FrontDeskContactNotConfigured</c> — never a Failed
/// Message, never a retry (mandate §19-21: this is expected, optional
/// configuration, not an infrastructure failure).
/// </summary>
public sealed class GuestCheckedInFrontDeskNotificationProcessor : IIntegrationEventHandler<GuestCheckedIn>
{
    ...
    private const string CheckedInAtVariable = "CheckedInAt";
    ...
    private readonly ILogger<GuestCheckedInFrontDeskNotificationProcessor> _logger;

    public GuestCheckedInFrontDeskNotificationProcessor(
        ...
        ILogger<GuestCheckedInFrontDeskNotificationProcessor> logger)
    {
    ...
    public async Task HandleAsync(GuestCheckedIn @event, CancellationToken cancellationToken)
    {
        ...
        var templateVariables = new Dictionary<string, string>
        {
            ...
            [CheckedInAtVariable] = @event.CheckedInAtUtc.UtcDateTime.ToString("yyyy-MM-dd HH:mm"),
        };
    ...
    private void LogSkipped(GuestCheckedIn @event, string reasonCode) =>
        _logger.LogInformation(
            ...
            nameof(GuestCheckedIn), @event.TenantId, @event.ReservationId, @event.PropertyId, "FrontDeskNotificationSkipped", reasonCode);

    private void LogResult(GuestCheckedIn @event, Message message) =>
        _logger.LogInformation(
            ...
            "SendFrontDeskGuestCheckedInNotification", nameof(GuestCheckedIn), "System", @event.TenantId, @event.ReservationId,
