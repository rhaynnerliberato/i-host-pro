...
/// indisponível) that had no reliable native AWS CloudWatch metric (the ALB
/// target group health check only probes <c>/health/live</c>, never
/// <c>/health/ready</c>). Mirrors <c>DeadLetterMetricsBackgroundService</c>'s
/// own established pattern exactly — a periodic poll into an in-memory
/// cache, read synchronously by an <see cref="ObservableGauge{T}"/>
/// callback, which must never itself touch a real dependency.
///
