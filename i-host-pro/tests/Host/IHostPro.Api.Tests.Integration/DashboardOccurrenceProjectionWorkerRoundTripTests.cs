...
/// <summary>
/// Second real transport gate mandated before closing Checkpoint 1 (Fase 7,
/// Incremento 2 — Dashboard &amp; Reporting Foundation, §44):
/// <c>CleaningOccurrenceRegistered</c> published through Housekeeping's own
/// real durable outbox, delivered over a real RabbitMQ broker, consumed by a
/// real, unmodified <c>IHostPro.Worker.dll</c> subprocess through
/// <c>IDashboardMessageExecutionScope</c> — proving <c>Type</c>,
/// <c>RegisteredAtUtc</c>, <c>CleaningId</c>, absence of Description/PII,
/// RLS, and idempotency. Mirrors
/// <c>DashboardReservationProjectionWorkerRoundTripTests</c> exactly.
/// </summary>
public sealed class DashboardOccurrenceProjectionWorkerRoundTripTests : IAsyncLifetime
{
    ...
    [Fact]
    public async Task CleaningOccurrenceRegistered_delivered_through_real_RabbitMQ_to_a_real_Worker_process_populates_the_Dashboard_projection_with_correct_tenant_isolation_and_idempotency()
    {
        ...
        var cleaningId = await SeedAssignedCleaningAsync(tenantId, propertyId, housekeeperUserId, now);

        StartWorkerProcess();
        var workerReady = await WaitForWorkerLogLineAsync(
            "Started message listening at rabbitmq://queue/dashboard.occurrence-projection", TimeSpan.FromSeconds(30));
        workerReady.Should().BeTrue("the real Worker must report listening to dashboard.occurrence-projection before the event is published");
        ...
        await using var envelopeProbe = await DeclareOccurrenceRegisteredProbeQueueAsync();
        ...
        try
        {
            ...
            using (var scope = factory.Services.CreateScope())
            {
                ...
                var dispatcher = scope.ServiceProvider.GetRequiredService<IHousekeepingRequestDispatcher>();

                var result = await dispatcher.Send(new RegisterCleaningOccurrenceCommand(
                    tenantId, housekeeperUserId, cleaningId, "Damage", "A door handle was broken"));
                result.IsSuccess.Should().BeTrue("the seeded Cleaning must be genuinely owned by this housekeeper and open");
            ...
            var projection = await WaitUntilNotNullAsync(
                () => ReadOccurrenceProjectionAsync(tenantId, occurrenceId), TimeSpan.FromSeconds(30));
            if (projection is null)
            {
                string workerOutputSnapshot;
                lock (_workerOutputLock) workerOutputSnapshot = _workerOutput.ToString();
                Assert.Fail("The real Worker must consume the real CleaningOccurrenceRegistered event and populate dashboard.occurrence_projection within 30s. Worker output:\n" + workerOutputSnapshot);
            }

            projection!.Value.CleaningId.Should().Be(cleaningId);
            projection.Value.Type.Should().Be("Damage");
            projection.Value.RegisteredAtUtc.Should().BeCloseTo(now, TimeSpan.FromMinutes(1));

            // ---- Cross-tenant isolation: the SAME occurrenceId, queried
            // under a DIFFERENT tenant's RLS context, must never resolve. ----
            (await ReadOccurrenceProjectionAsync(otherTenantId, occurrenceId)).Should().BeNull(
            ...
            captured.Should().NotBeNull("the probe, bound to the same routing key as dashboard.occurrence-projection, must have received an identical copy of the published CleaningOccurrenceRegistered");
            ...
            await envelopeProbe.Channel.BasicPublishAsync(
                ...
                routingKey: "dashboard.occurrence-projection",
                mandatory: false,
            ...
            (await CountOccurrenceProjectionRowsAsync(tenantId, occurrenceId)).Should().Be(1,
                "redelivery of the same CleaningOccurrenceRegistered must never create a duplicate row (idempotent by construction — checks existence before insert)");
        }
        finally
        {
    ...
    private async Task<Guid> SeedAssignedCleaningAsync(Guid tenantId, Guid propertyId, Guid housekeeperUserId, DateTimeOffset now)
    {
        var cleaning = Cleaning.Create(Guid.NewGuid(), tenantId, propertyId, reservationId: null, createdByUserId: Guid.NewGuid(), now);
        cleaning.Assign(housekeeperUserId, now);
    ...
    private async Task<(Guid CleaningId, string Type, DateTimeOffset RegisteredAtUtc)?> ReadOccurrenceProjectionAsync(
        Guid tenantId, Guid occurrenceId)
    {
        ...
        command.CommandText = "SELECT cleaning_id, type, registered_at_utc FROM dashboard.occurrence_projection WHERE tenant_id = @tenantId AND occurrence_id = @id";
        command.Parameters.AddWithValue("tenantId", tenantId);
        command.Parameters.AddWithValue("id", occurrenceId);

        (Guid, string, DateTimeOffset)? result = null;
        await using (var reader = await command.ExecuteReaderAsync())
        {
            if (await reader.ReadAsync())
                result = (reader.GetGuid(0), reader.GetString(1), reader.GetFieldValue<DateTimeOffset>(2));
        }
    ...
    private async Task<long> CountOccurrenceProjectionRowsAsync(Guid tenantId, Guid occurrenceId)
    {
        await using var connection = new NpgsqlConnection(_migratorConnectionString);
        await connection.OpenAsync();
        await using var transaction = await connection.BeginTransactionAsync();
        ...
        await using var command = connection.CreateCommand();
        command.CommandText = "SELECT count(*) FROM dashboard.occurrence_projection WHERE tenant_id = @tenantId AND occurrence_id = @id";
        command.Parameters.AddWithValue("tenantId", tenantId);
        command.Parameters.AddWithValue("id", occurrenceId);
    ...
    private async Task<RabbitMqProbe> DeclareOccurrenceRegisteredProbeQueueAsync()
    {
        var connection = await CreateProbeConnectionAsync();
        var channel = await connection.CreateChannelAsync();

        var queue = $"test-dashboard-occurrence-registered-probe-{Guid.NewGuid():N}";
        await channel.QueueDeclareAsync(queue, durable: false, exclusive: true, autoDelete: true);
        await channel.QueueBindAsync(queue, "housekeeping-events", "cleaning_occurrence_registered");
