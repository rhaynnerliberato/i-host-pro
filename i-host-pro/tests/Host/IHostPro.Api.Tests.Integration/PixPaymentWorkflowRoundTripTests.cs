...
public sealed class PixPaymentWorkflowRoundTripTests : IClassFixture<PixPaymentWorkflowRoundTripTests.Fixture>
{
    ...
    public PixPaymentWorkflowRoundTripTests(Fixture fixture) => _fixture = fixture;
    ...
    public sealed class Fixture : IAsyncLifetime
    {
        ...
        /// <summary>
        /// Publishes <see cref="PixChargeConfirmationReceived"/> onto the
        /// real RabbitMQ broker via a throwaway, send-only Wolverine host —
        /// simulating the provider-neutral confirmation fact a future
        /// ExternalIntegrations webhook-normalization step would produce
        /// (approved decision — never a test-only HTTP endpoint). Uses the
        ...
        public async Task PublishPixChargeConfirmationReceivedAsync(PixChargeConfirmationReceived message)
        {
            ...
            senderBuilder.Configuration["RabbitMq:Username"] = RabbitMqBuilder.DefaultUsername;
            senderBuilder.Configuration["RabbitMq:Password"] = RabbitMqBuilder.DefaultPassword;

            using var senderHost = senderBuilder.Build();
            await senderHost.StartAsync();
            try
            {
                using var scope = senderHost.Services.CreateScope();
    ...
    // ==================================================================
    // 1. PAYMENT CREATION — LateCheckoutPaymentRequired -> PixCharge -> PixChargeCreated -> guest delivery
    // ==================================================================

    [Fact]
    public async Task LateCheckout_requiring_Pix_creates_a_real_PixCharge_and_delivers_the_QR_to_the_guest_never_the_Integration_Event()
    {
        ...
        await SeedTemplateAsync(tenantId, "LATE_CHECKOUT_PIX_PAYMENT", "Ola {{GuestName}}, pague {{Amount}} via PIX: {{PixCode}}");
        ...
        await SeedLateCheckoutPolicyAsync(tenantId,
            """{"allowed":true,"latestTime":null,"chargeType":"fixedAmount","chargeValue":75.00,"requiresPix":true,"blocksCalendar":false,"updatesCleaning":false}""");
        ...
        chargeCreated.Should().BeTrue(
            "the real LateCheckoutPaymentRequired -> Payments -> FakePixProvider chain must create a PixCharge within 30s. " +
            "Worker output:\n" + _fixture.GetWorkerOutputSnapshot());

        var charge = (await GetPixChargeAsync(tenantId, lateCheckoutRequestId))!.Value;
        charge.Status.Should().Be("Pending");
        charge.Amount.Should().Be(75.00m);
        charge.CurrencyCode.Should().Be("BRL");
        charge.ProviderChargeId.Should().NotBeNullOrEmpty("FakePixProvider always accepts deterministically");
        charge.QrCodePayload.Should().NotBeNullOrEmpty("the QR payload must be persisted at creation time — ADR-025");

        // ---- Guest delivery: proves the QR reaches the GUEST, never the front desk ----
        var messageCreated = await WaitUntilAsync(
            () => GetMessageAsync(tenantId, reservationId, "LATE_CHECKOUT_PIX_PAYMENT"), m => m is not null, TimeSpan.FromSeconds(30));
        messageCreated.Should().BeTrue(
            "Communication's PIX delivery processor must react to PixChargeCreated within 30s. Worker output:\n" + _fixture.GetWorkerOutputSnapshot());

        var message = (await GetMessageAsync(tenantId, reservationId, "LATE_CHECKOUT_PIX_PAYMENT"))!.Value;
        message.Status.Should().Be("Sent", "FakeWhatsAppConnector always succeeds");
        message.DestinationMasked.Should().EndWith("8888", "the recipient must be the GUEST phone, ending in the guest's own last 4 digits");
        message.RenderedContent.Should().Contain(charge.QrCodePayload,
            "the guest message body is the QR's intended final destination — never a leak, unlike logs/events/query strings");

        // ---- Structural proof: the QR payload never crossed via the Integration Event ----
        // PixChargeCreated's own schema (Payments.Contracts) has no
        // QrCodePayload/ProviderChargeId property at all — see
        // PaymentsDependencyTests.Payments_Integration_Events_Never_Carry_QR_Provider_Or_Payer_Data
        // for the architecture-level guarantee; this E2E additionally
        // confirms the RUNTIME value the guest received matches the
        // PERSISTED PixCharge row, never a value that could only have come
        // from message-bus payload inspection.
    }

    // ==================================================================
    // 2. PAYMENT CONFIRMATION — the main acceptance proof (mandate item 54)
    // ==================================================================

    [Fact]
    public async Task PixChargeConfirmationReceived_approves_the_LateCheckoutRequest_and_fans_out_to_Workflow_Housekeeping_and_Communication()
    {
        var tenantId = Guid.NewGuid();
        var now = DateTimeOffset.UtcNow;
        var checkInAt = now.AddDays(5);
        ...
        var chargeCreated = await WaitUntilAsync(
            () => GetPixChargeAsync(tenantId, lateCheckoutRequestId), c => c is not null, TimeSpan.FromSeconds(30));
        chargeCreated.Should().BeTrue("PixCharge must be created before it can be confirmed. Worker output:\n" + _fixture.GetWorkerOutputSnapshot());
        var pixChargeId = (await GetPixChargeAsync(tenantId, lateCheckoutRequestId))!.Value.Id;

        // ---- Simulate the provider-neutral confirmation fact ----
        ...
        confirmed.Should().BeTrue("PixCharge must reach Confirmed within 30s. Worker output:\n" + _fixture.GetWorkerOutputSnapshot());
        ...
        approved.Should().BeTrue(
            "PixChargeConfirmed -> Guest Operations must approve the LateCheckoutRequest within 30s. Worker output:\n" + _fixture.GetWorkerOutputSnapshot());
        ...
        rescheduled.Should().BeTrue("Workflow must reschedule the real Reservation. Worker output:\n" + _fixture.GetWorkerOutputSnapshot());
        ...
        auditRecorded.Should().BeTrue("Housekeeping must react to the PIX-confirmation-triggered approval. Worker output:\n" + _fixture.GetWorkerOutputSnapshot());

        var messageCreated = await WaitUntilAsync(
            () => GetMessageAsync(tenantId, reservationId, "FRONT_DESK_LATE_CHECKOUT_APPROVED"), m => m is not null, TimeSpan.FromSeconds(30));
        messageCreated.Should().BeTrue("Communication's Front Desk processor must react. Worker output:\n" + _fixture.GetWorkerOutputSnapshot());
        (await GetMessageAsync(tenantId, reservationId, "FRONT_DESK_LATE_CHECKOUT_APPROVED"))!.Value.Status.Should().Be("Sent");
    }

    // ==================================================================
    // 3. DUPLICATE CONFIRMATION — idempotency (mandate item 55)
    // ==================================================================

    [Fact]
    public async Task Duplicate_PixChargeConfirmationReceived_produces_zero_duplicate_business_effects()
    {
        var tenantId = Guid.NewGuid();
        var now = DateTimeOffset.UtcNow;
        var checkInAt = now.AddDays(5);
        ...
        var confirmationCorrelationId = Guid.NewGuid();
        var confirmedAtUtc = DateTimeOffset.UtcNow;

        // Publish the SAME confirmation fact twice.
        for (var i = 0; i < 2; i++)
        {
            await _fixture.PublishPixChargeConfirmationReceivedAsync(new PixChargeConfirmationReceived
            {
        ...
        var approved = await WaitUntilAsync(
            () => GetLateCheckoutRequestStatusAsync(tenantId, lateCheckoutRequestId), status => status == "Approved", TimeSpan.FromSeconds(30));
        approved.Should().BeTrue("Worker output:\n" + _fixture.GetWorkerOutputSnapshot());
        ...
        // Settle window so a wrongly-duplicated second reaction has time to land before asserting its absence.
        await Task.Delay(TimeSpan.FromSeconds(5));

        (await CountCleaningAuditEntriesAsync(tenantId, cleaningId, "late_checkout_approved")).Should().Be(1,
            "exactly one audit entry — a duplicate confirmation delivery must never duplicate Housekeeping's own side effect");
    ...
    private Task<string?> GetLateCheckoutRequestStatusAsync(Guid tenantId, Guid lateCheckoutRequestId) =>
        QueryScopedAsync(tenantId, async connection =>
        {
            await using var command = connection.CreateCommand();
            command.CommandText = "SELECT status FROM guest_operations.late_checkout_requests WHERE tenant_id = @tenantId AND id = @id";
            command.Parameters.AddWithValue("tenantId", tenantId);
            command.Parameters.AddWithValue("id", lateCheckoutRequestId);
            return (await command.ExecuteScalarAsync()) as string;
        });
    ...
    private Task<(Guid Id, string Status, decimal Amount, string CurrencyCode, string? ProviderChargeId, string? QrCodePayload)?> GetPixChargeAsync(
        Guid tenantId, Guid lateCheckoutRequestId) =>
        QueryScopedAsync(tenantId, async connection =>
        {
            await using var command = connection.CreateCommand();
            command.CommandText = """
                SELECT id, status, amount, currency_code, provider_charge_id, qr_code_payload FROM payments.pix_charges
                WHERE tenant_id = @tenantId AND late_checkout_request_id = @lateCheckoutRequestId
                """;
            command.Parameters.AddWithValue("tenantId", tenantId);
            command.Parameters.AddWithValue("lateCheckoutRequestId", lateCheckoutRequestId);

            await using var reader = await command.ExecuteReaderAsync();
            if (!await reader.ReadAsync())
                return ((Guid, string, decimal, string, string?, string?)?)null;

            return (
                reader.GetGuid(0), reader.GetString(1), reader.GetDecimal(2), reader.GetString(3),
                reader.IsDBNull(4) ? null : reader.GetString(4), reader.IsDBNull(5) ? null : reader.GetString(5));
        });

    private Task<string?> GetPixChargeStatusAsync(Guid tenantId, Guid pixChargeId) =>
        QueryScopedAsync(tenantId, async connection =>
        {
            await using var command = connection.CreateCommand();
            command.CommandText = "SELECT status FROM payments.pix_charges WHERE tenant_id = @tenantId AND id = @id";
            command.Parameters.AddWithValue("tenantId", tenantId);
            command.Parameters.AddWithValue("id", pixChargeId);
            return (await command.ExecuteScalarAsync()) as string;
        });
