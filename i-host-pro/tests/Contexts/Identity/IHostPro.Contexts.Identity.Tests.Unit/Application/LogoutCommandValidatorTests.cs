...
{
    private static readonly LogoutCommandValidator Validator = new();

    private static LogoutCommand Valid() => new(Guid.NewGuid(), Guid.NewGuid(), Guid.NewGuid());
    ...
    {
        var command = new LogoutCommand(Guid.Empty, Guid.Empty, Guid.Empty);
