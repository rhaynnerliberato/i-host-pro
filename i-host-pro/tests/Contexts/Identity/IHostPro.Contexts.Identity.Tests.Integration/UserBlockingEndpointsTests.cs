...
/// <summary>
/// End-to-end test of the two block/unblock endpoints (Incremento 3,
/// Checkpoint 7) — <c>POST /api/v1/users/{userId}/block(/unblock)</c> on
/// <see cref="UserAdministrationController"/> — against the REAL composition
/// root wiring, mirroring <see cref="UserRoleAssignmentEndpointsTests"/>'s
/// shape exactly (own Postgres+Redis Fixture, real <c>AddIdentityCommandDispatch</c>,
/// no RabbitMQ — event content/routing already covered by
/// <see cref="IdentityIntegrationEventsTests"/>). Additionally exercises the
/// real <c>/api/v1/auth/login</c> endpoint to prove the block/unblock
/// lifecycle's effect on authentication itself (Section 9).
/// </summary>
public class UserBlockingEndpointsTests : IClassFixture<UserBlockingEndpointsTests.Fixture>
{
    ...
    public UserBlockingEndpointsTests(Fixture fixture)
    {
        ...
        var hostBuilder = new HostBuilder()
            .ConfigureWebHost(webHost =>
            {
                ...
                webHost.ConfigureServices(services =>
                {
                    ...
                    // AuthController (needed transitively by the login test
    ...
    [Fact]
    public async Task Block_without_a_token_returns_401()
    {
        using var host = await BuildHostAsync();
        using var client = host.GetTestClient();

        var response = await PostAsync(client, $"/api/v1/users/{Guid.NewGuid()}/block", null, token: null);

        response.StatusCode.Should().Be(HttpStatusCode.Unauthorized);
    }

    [Fact]
    public async Task Unblock_without_a_token_returns_401()
    {
        using var host = await BuildHostAsync();
        using var client = host.GetTestClient();

        var response = await PostAsync(client, $"/api/v1/users/{Guid.NewGuid()}/unblock", null, token: null);
    ...
    [Fact]
    public async Task Block_with_a_role_lacking_USERS_MANAGE_returns_403()
    {
        ...
        var response = await PostAsync(client, $"/api/v1/users/{targetUserId}/block", null, token);
    ...
    [Fact]
    public async Task Admin_blocks_a_user_and_receives_204_with_no_store_header()
    {
        var (tenantId, _) = await SeedTenantAsync();
        var (actorId, _) = await SeedUserAsync(tenantId);
        var (targetUserId, _) = await SeedUserAsync(tenantId);
    ...
    [Fact]
    public async Task Block_for_a_user_of_a_different_tenant_returns_404()
    {
        ...
        var response = await PostAsync(client, $"/api/v1/users/{userInTenantB}/block", null, token);

        response.StatusCode.Should().Be(HttpStatusCode.NotFound);
    }

    [Fact]
    public async Task Block_of_an_already_blocked_user_returns_409()
    {
        var (tenantId, _) = await SeedTenantAsync();
        var (actorId, _) = await SeedUserAsync(tenantId);
        var (targetUserId, _) = await SeedUserAsync(tenantId);
        ...
        var actorSessionId = await SeedSessionAsync(tenantId, actorId);
        var token = await GenerateTokenAsync(host, actorId, tenantId, actorSessionId, ["ADMIN"]);
        (await PostAsync(client, $"/api/v1/users/{targetUserId}/block", null, token)).StatusCode.Should().Be(HttpStatusCode.NoContent);
    ...
    [Fact]
    public async Task Block_of_the_tenants_last_active_Administrator_returns_409()
    {
        var (tenantId, _) = await SeedTenantAsync();
        var (actorId, _) = await SeedUserAsync(tenantId);
        var (targetUserId, _) = await SeedUserAsync(tenantId);
    ...
    [Fact]
    public async Task Admin_unblocks_a_user_and_receives_204_with_no_store_header()
    {
        var (tenantId, _) = await SeedTenantAsync();
        var (actorId, _) = await SeedUserAsync(tenantId);
        var (targetUserId, _) = await SeedUserAsync(tenantId);
        ...
        var response = await PostAsync(client, $"/api/v1/users/{targetUserId}/unblock", null, token);
    ...
    [Fact]
    public async Task Unblock_of_an_already_active_user_returns_409()
    {
        var (tenantId, _) = await SeedTenantAsync();
        var (actorId, _) = await SeedUserAsync(tenantId);
        var (targetUserId, _) = await SeedUserAsync(tenantId);
    ...
    // ---- Tests: effect on authentication (Section 9) ------------------------

    [Fact]
    public async Task After_Block_the_targets_old_access_token_is_rejected_with_Redis_available()
    {
        var (tenantId, _) = await SeedTenantAsync();
        var (actorId, _) = await SeedUserAsync(tenantId);
        var (targetUserId, _) = await SeedUserAsync(tenantId);
        ...
        var blockResponse = await PostAsync(client, $"/api/v1/users/{targetUserId}/block", null, actorToken);
    ...
    [Fact]
    public async Task A_blocked_user_cannot_log_in()
    {
        ...
        var loginResponse = await LoginAsync(client, slug, targetEmail);

        loginResponse.StatusCode.Should().Be(HttpStatusCode.Unauthorized);
    }

    [Fact]
    public async Task After_Unblock_the_user_can_log_in_again_but_old_tokens_remain_invalid()
    {
        var (tenantId, slug) = await SeedTenantAsync();
        var (actorId, _) = await SeedUserAsync(tenantId);
        var (targetUserId, targetEmail) = await SeedUserAsync(tenantId);
        ...
        var actorSessionId = await SeedSessionAsync(tenantId, actorId);
        var actorToken = await GenerateTokenAsync(host, actorId, tenantId, actorSessionId, ["ADMIN"]);
        var targetOldSessionId = await SeedSessionAsync(tenantId, targetUserId);
        var targetOldToken = await GenerateTokenAsync(host, targetUserId, tenantId, targetOldSessionId, ["HOUSEKEEPER"]);
        (await PostAsync(client, $"/api/v1/users/{targetUserId}/block", null, actorToken)).StatusCode.Should().Be(HttpStatusCode.NoContent);

        var unblockResponse = await PostAsync(client, $"/api/v1/users/{targetUserId}/unblock", null, actorToken);
        unblockResponse.StatusCode.Should().Be(HttpStatusCode.NoContent);

        var loginResponse = await LoginAsync(client, slug, targetEmail);
        loginResponse.StatusCode.Should().Be(HttpStatusCode.OK);

        // The session/token that existed BEFORE the block stays revoked —
        // unblocking never reactivates it, only a brand-new login (above)
        // creates a usable session.
        var oldTokenFollowUp = await GetAsync(client, $"/api/v1/users/{targetUserId}", targetOldToken);
        oldTokenFollowUp.StatusCode.Should().Be(HttpStatusCode.Unauthorized);
    }
