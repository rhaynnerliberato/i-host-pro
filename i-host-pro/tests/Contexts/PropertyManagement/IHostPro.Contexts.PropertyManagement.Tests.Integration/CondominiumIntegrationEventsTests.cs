...
/// <summary>
/// Validates Property Management's durable outbox end-to-end against real
/// PostgreSQL and RabbitMQ (Testcontainers) — mirrors
/// <c>IdentityOutboxTransactionExecutorTests</c>: same-commit envelope
...
/// (Checkpoint 2 plan, item 16). Dispatches through the REAL production
/// composition root (<c>AddPropertyManagementModule</c>/
/// <c>AddPropertyManagementCommandDispatch</c>) via <see cref="ISender"/>.
/// </summary>
public class CondominiumIntegrationEventsTests : IClassFixture<CondominiumIntegrationEventsTests.Fixture>
{
    ...
    public CondominiumIntegrationEventsTests(Fixture fixture)
    {
    ...
    public sealed class Fixture : IAsyncLifetime
    {
        ...
        /// schema to query against (not merely a schema that doesn't exist at
        /// all, which would trivially return zero rows for the wrong reason).
        /// </summary>
    ...
    private async Task<IHost> BuildHostAsync(RabbitMqContainer rabbitMqContainer)
    {
        ...
        hostBuilder.UseWolverine(opts =>
        {
            ...
            // agent — mirrors IdentityOutboxTransactionExecutorTests exactly
            ...
            opts.PublishMessage(typeof(CondominiumCreated))
                .ToRabbitRoutingKey(ExchangeName, "condominium_created", exchange => exchange.ExchangeType = ExchangeType.Topic)
                .UseDurableOutbox();
            opts.PublishMessage(typeof(CondominiumUpdated))
                .ToRabbitRoutingKey(ExchangeName, "condominium_updated", exchange => exchange.ExchangeType = ExchangeType.Topic)
                .UseDurableOutbox();
        });
    ...
    /// <summary>
    /// RabbitMQ is stopped BEFORE the command runs — not to test the outage
    /// path (that is <see cref="RabbitMQ_unavailable_does_not_block_the_commit_and_the_envelope_is_delivered_after_recovery"/>'s
    /// job), but because Wolverine's first delivery attempt after commit is
    /// synchronous and near-immediate when the broker IS reachable (see
    /// <c>Program.cs</c>'s own <c>CircuitBreaking</c> comment) — confirmed
    /// empirically that this raced the very envelope-existence assertion
    /// below to zero rows. Stopping the broker first is the same technique
    /// <c>IdentityOutboxTransactionExecutorTests</c> uses to keep an envelope
    /// reliably visible for inspection.
    /// </summary>
    [Fact]
    public async Task Creating_a_condominium_persists_a_CondominiumCreated_envelope_in_property_management_messaging_only()
    {
        ...
        try
        {
            ...
            try
            {
                ...
                (await CountEnvelopesAsync(OutboxSchema, "IHostPro.Contexts.PropertyManagement.Contracts.CondominiumCreated")).Should().Be(1);
                (await CountEnvelopesAsync("identity_messaging", "IHostPro.Contexts.PropertyManagement.Contracts.CondominiumCreated")).Should().Be(0);
            }
            finally
            {
    ...
    [Fact]
    public async Task Updating_a_condominium_persists_a_CondominiumUpdated_envelope_with_ChangedFields()
    {
        var rabbitMqContainer = new RabbitMqBuilder().WithImage("rabbitmq:3-management-alpine").Build();
        await rabbitMqContainer.StartAsync();
        try
        {
            ...
            try
            {
                ...
                result.IsSuccess.Should().BeTrue();
                (await CountEnvelopesAsync(OutboxSchema, "IHostPro.Contexts.PropertyManagement.Contracts.CondominiumUpdated")).Should().Be(1);
            }
            finally
            {
                await StopGracefullyAsync(host);
    ...
    public async Task RabbitMQ_unavailable_does_not_block_the_commit_and_the_envelope_is_delivered_after_recovery()
    {
        ...
        try
        {
            ...
            try
            {
                ...
                (await dbContext.Condominiums.CountAsync(c => c.Id == condominiumId)).Should().Be(1);

                (await EnvelopeIsPendingAsync(condominiumId)).Should().BeTrue();
            }
            finally
            {
                await StopGracefullyAsync(host);
            ...
            // Recovery is verified with a NEW host/connection — mirrors
            // IdentityOutboxTransactionExecutorTests exactly (see its own doc
            // comment for why a restarted broker needs a fresh connection).
            var recoveryHost = await BuildHostAsync(rabbitMqContainer);
            try
            {
                // A delivered envelope is removed from wolverine_outgoing_envelopes
                // entirely (it is a pending-delivery queue, not a permanent
                // log) — "no longer pending" is therefore the correct signal
                // of successful delivery, not an envelope row to keep
                // counting. Confirmed empirically here.
                await WaitUntilAsync(async () => !await EnvelopeIsPendingAsync(condominiumId), TimeSpan.FromSeconds(30));
            }
            finally
            {
            ...
            // Distinguishes a genuine successful delivery from a silent drop:
            // a delivery Wolverine gives up on lands in wolverine_dead_letters
            // instead of just vanishing from wolverine_outgoing_envelopes.
    ...
    private async Task<bool> EnvelopeIsPendingAsync(Guid condominiumId)
    {
        await using var connection = new NpgsqlConnection(_migratorConnectionString);
        await connection.OpenAsync();
        await using var command = connection.CreateCommand();
        command.CommandText = $"""
            ...
            WHERE message_type = 'IHostPro.Contexts.PropertyManagement.Contracts.CondominiumCreated'
              AND position(convert_to(@condominiumId, 'UTF8') in body) > 0
            """;
        command.Parameters.AddWithValue("condominiumId", condominiumId.ToString());
