...
/// <summary>
/// Fase 10, Checkpoint 4 (Portaria Notification Foundation) mandate §42 —
/// authentication/authorization proof for <see cref="FrontDeskContactsController"/>
/// against the REAL composition root wiring — mirrors
/// <c>CondominiumsEndpointsTests</c>'s own fixture exactly (Postgres only, no
/// RabbitMQ; real JWT issuance/validation; real seeded permission catalog).
/// </summary>
public class FrontDeskContactsEndpointsTests : IClassFixture<FrontDeskContactsEndpointsTests.Fixture>
{
    ...
    public FrontDeskContactsEndpointsTests(Fixture fixture)
    {
    ...
    private static async Task<Guid> CreateCondominiumAsync(HttpClient client, string token)
    {
        var response = await PostAsync(client, "/api/v1/condominiums", new CreateCondominiumRequest("Condominio Teste", ValidAddress), token);
    ...
    public async Task Set_without_a_token_returns_401()
    {
        ...
        var response = await PutAsync(client, $"/api/v1/condominiums/{Guid.NewGuid()}/front-desk-contact",
            new SetFrontDeskContactRequest("Portaria Bloco A", "+5511977776666", true), token: null);
    ...
    public async Task Set_with_a_role_lacking_PROPERTIES_MANAGE_returns_403()
    {
        ...
        var response = await PutAsync(client, $"/api/v1/condominiums/{Guid.NewGuid()}/front-desk-contact",
            new SetFrontDeskContactRequest("Portaria Bloco A", "+5511977776666", true), token);

        response.StatusCode.Should().Be(HttpStatusCode.Forbidden);
    }

    [Fact]
    public async Task Set_as_ADMIN_succeeds_and_Get_returns_it_back()
    {
        using var host = await BuildHostAsync();
        using var client = host.GetTestClient();
        ...
        var setResponse = await PutAsync(client, $"/api/v1/condominiums/{condominiumId}/front-desk-contact",
            new SetFrontDeskContactRequest("Portaria Bloco A", "+5511977776666", true), token);

        setResponse.StatusCode.Should().Be(HttpStatusCode.OK);
        var setBody = await setResponse.Content.ReadFromJsonAsync<FrontDeskContactResponse>(JsonWebDefaults);
        setBody!.DisplayName.Should().Be("Portaria Bloco A");
        setBody.PhoneNumber.Should().Be("+5511977776666");
        setBody.IsActive.Should().BeTrue();

        var getResponse = await GetAsync(client, $"/api/v1/condominiums/{condominiumId}/front-desk-contact", token);
        getResponse.StatusCode.Should().Be(HttpStatusCode.OK);
        var getBody = await getResponse.Content.ReadFromJsonAsync<FrontDeskContactResponse>(JsonWebDefaults);
        getBody!.Id.Should().Be(setBody.Id);
    }

    [Fact]
    public async Task Set_for_a_nonexistent_condominium_returns_404()
    {
        using var host = await BuildHostAsync();
        using var client = host.GetTestClient();
    ...
    [Fact]
    public async Task Get_before_any_contact_is_configured_returns_404()
    {
        using var host = await BuildHostAsync();
        using var client = host.GetTestClient();
        var token = await GenerateTokenAsync(host, Guid.NewGuid(), Guid.NewGuid(), ["ADMIN"]);
        ...
        var response = await GetAsync(client, $"/api/v1/condominiums/{condominiumId}/front-desk-contact", token);

        response.StatusCode.Should().Be(HttpStatusCode.NotFound);
    }

    [Fact]
    public async Task A_condominium_from_another_tenant_is_not_found()
    {
        using var host = await BuildHostAsync();
        using var client = host.GetTestClient();
        var ownerToken = await GenerateTokenAsync(host, Guid.NewGuid(), Guid.NewGuid(), ["ADMIN"]);
        var condominiumId = await CreateCondominiumAsync(client, ownerToken);
        await PutAsync(client, $"/api/v1/condominiums/{condominiumId}/front-desk-contact",
            new SetFrontDeskContactRequest("Portaria Bloco A", "+5511977776666", true), ownerToken);

        var otherTenantToken = await GenerateTokenAsync(host, Guid.NewGuid(), Guid.NewGuid(), ["ADMIN"]);

        var response = await GetAsync(client, $"/api/v1/condominiums/{condominiumId}/front-desk-contact", otherTenantToken);

        response.StatusCode.Should().Be(HttpStatusCode.NotFound, "a cross-tenant condominiumId must be indistinguishable from a non-existent one (RLS)");
    }
}
